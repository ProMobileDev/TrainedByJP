...
-(void)callApi
{
    ...
    NSDictionary *params = [UrlManager getCategoryParameters:13];
    ...
    [[ApiManager sharedInstance].manager POST:BASE_URL parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
     {
         ...
         {
             self.articlesArray = [dict objectForKey:@"posts"];
         }
...
{
    [ApiManager sharedInstance].selectedDict = [self.articlesArray objectAtIndex:indexPath.row];
